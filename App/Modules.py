...
class SchemaLinking():
    ...
    def __init__(self, domain):
        ...
        self.table_descriptions_vec = dict()

        for table_name, table_info in domain['tables'].items():
            self.table_descriptions_vec[table_name] = table_info['description']['vector']
    ...
    def table_selected(self, question:str, max_cols:int=30):
        ...
        for table, table_vector in self.table_descriptions_vec.items():
            ...
            total_sum += num_cols
            if table in question_split:
                table_scores[table] = 1
            else: table_scores[table] = round(float(cos_sim(table_vector, question_vector)),3)
        ...
        for table in domain_tables:
            cols_selected += total_cols[table]
            if first_iteration:
                selected_tables.append(table)
                first_iteration = False
            elif cols_selected <= max_cols:
                selected_tables.append(table)
            else:
                break
